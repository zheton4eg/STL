...
const std::map<int, std::string> VIOLATIONS =
{
	...
	{6, "Отсутствие права управления авто"},
	{7, "Отстутствие страховки"},
	{8, "Проезд на красный"},
	...
	{16, "Перевозка ребёнка без кресла"},
};
...
public:
	...
	{
		//1)Создаём временную строку для того чтобы пропарсить полученную строку:
		char* time_buffer = new char[time.size() + 1] {};
		//2) Копирум полученную строку в буфер:
		strcpy(time_buffer, time.c_str());
		//Функция strcpu(dst, src); копируем содержимое строки источника (src- Source) в строку получателя(dst

		//3) Создаём массив для храниения элементов времени:
		int time_elements[5]{};
		int i = 0;
		char delimiters[] = ":./ ";
		...
		//4) Сохраняем элементы в строку 'tm':
...
void main()
{
	...
	std::map<std::string, std::list<Crime>> base =
	{
		...
		{"a001àà", {Crime(10, "ул. Пролетарская", "21:50 1.08.2024"), Crime(9,"ул. Пролетарская", "21:50 1.08.2024"), Crime(11,"ул. Пролетарская", "21:50 1.08.2024"), Crime(12,"ул. Пролетарская", "22:05 1.08.2024")}},
	};
	print(base);
	save(base, "base.txt");
#endif // SAVE_CHECK
...
void print(const std::map<std::string, std::list<Crime>>& base)
{
	...
	cout << "Кол-во номеров в базе: " << base.size() << endl;
	system("PAUSE");
}
void save(const std::map<std::string, std::list<Crime>>& base, const std::string& filename)
{
	...
	if (fin.is_open())
	{
		while (!fin.eof())
		{
			...
			for (char* pch = strtok(sz_buffer, delimiters); pch; pch = strtok(NULL, delimiters))
			{
				...
				//Crime crime(0, "place", "00:00 01.01.2000");	//При создании этого обьекта локально 'for',
																//Он нарушает работу функции strtok(), поэтому обьект создается в;
